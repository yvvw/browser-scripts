...
// @version      0.0.7
// @description  调整屏宽，移除buy more，增加bullx跳转
// @author       yvvw
...
import { observe, parseQuery, waitingElement } from './util'

const cleanSet = new Set<() => void>()
...
async function addBullLink() {
  const table = await waitingElement(() =>
    document.querySelector<HTMLTableElement>('.g-table-tbody')
  )

  for (const row of table.children) {
    if ((row as HTMLElement).dataset['modified'] === '1') {
      continue
    }
    ;(row as HTMLElement).dataset['modified'] = '1'

    const linkEL = row.querySelector<HTMLAnchorElement>('a')
    if (linkEL === null) {
    ...
    const divEl = row.querySelector(`div[title="${symbol}"]`)
    if (divEl === null) {
...
function createBullXEl(path: string) {
  ...
  const href = `https://bullx.io/terminal?chainId=${chainId}&address=${address}`
...
async function adjustRecordSize() {
  await waitingElement(() => document.getElementById('tokenCenter'))
...
async function removeBuyMoreActivities() {
  const activityBtnEl = await waitingElement(
    () =>
  ...
  const container = await waitingElement(() => document.getElementById('gmgnCalls'))
